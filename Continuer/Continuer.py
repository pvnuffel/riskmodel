...
#            points[i].extra_condition+=params['extra_condition']    
    ...
    def bcontinue(self,nb_points): 
        ...
        while kontinue and tries <= nb_points:
            ...
            secant = last_point.axpy(-1,prev_point)  # secant = prev_point - last_point
            dist = scipy.linalg.norm(secant.u)+ scipy.linalg.norm(secant.lambd) #pseudo-arclength? ???? 
            ...
            new_point = secant.axpy(-steplength/dist,last_point)  # new_point = -steplength/dist*(prev_point - last_point) + lastpoint
            new_point.secant = {}
