...
class Precond(object):
    ...
    def computeJacobian(self,N,dx):
        ...
        for i in range(1,N-1):
            J[i,i-1] =Dstar/dx**2 + kappa[i-1]/(2*dx)      
            J[i,i]   =-2*Dstar/dx**2 + sigma[i]-(kappa[i]-kappa[i-1])/(2*dx)
            J[i,i+1] =Dstar/dx**2 - kappa[i]/(2*dx)     
                                  
        # Homogeneous Dirichlet boundary conditions with boundary points eliminated
        
        # left boundary
        J[0,0]   =-2*Dstar/dx**2 + sigma[0] -kappa[0]/(2*dx)
        J[0,1] =Dstar/dx**2 - kappa[0]/(2*dx)
        
        # right boundary
        J[N-1,N-2] = Dstar/dx**2 + kappa[N-2]/(2*dx)  
        J[N-1,N-1]   = -2*Dstar/dx**2 + sigma[N-1] + kappa[N-2]/(2*dx)
        return J
