...
#   rho_Dt_sde = E_rho[-1][-1]
  ...
  #  resize_factor = int (len(rho_Dt_fp)/len(rho_Dt_sde))
    resize_factor = int (len(Jv_pde)/len(Jv_sde))
  #  resize_factor = 10
 ...
 #   points_o1var = [[1e-5, 1e-2], [2e-5, 2e-2], [1e-5, 2e-2]] 
  ...
  #  plt.gca().add_patch(triangle_o1a)
   ...
   # plt.legend(bbox_to_anchor=(0.32, 1), numpoints = 1 )
    ...
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    for i in range (0,len(Nlist)):
        plot_bias_sq = plt.plot(Mlist, bias_sq[:,i] /bins, lines[i], label =r'$N=%d$' %Nlist[i], linewidth=2  )    #
  ...
  #  if(save_flag): plt.savefig("plots/25-11Bias_N.pdf")
